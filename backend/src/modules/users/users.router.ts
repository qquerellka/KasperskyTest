...
import { UserCreateDtoSchema, UserPatchDtoSchema } from "./users.dto.js";
...
usersRouter.get("/:id", async (req, res, next) => {
  try {
    ...
    const { memberships: _, ...rest } = u as any;
...
usersRouter.post("/", async (req, res, next) => {
  try {
    const data = UserCreateDtoSchema.parse(req.body);
...
usersRouter.patch("/:id", async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    const data = UserPatchDtoSchema.parse(req.body);
